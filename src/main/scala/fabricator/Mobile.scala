...
case class Mobile(private val random: Random = new Random(),
  ...
  def wp8_anid2: String = new sun.misc.BASE64Encoder().encode(alpha.randomString.getBytes("UTF-8"))

  def wp7_anid: String = "A=" + alpha.randomGuid.replace("-", "").toUpperCase + "&E=" + alpha.randomHash(3) + "&W=" + alpha.randomInt(0, 9)
