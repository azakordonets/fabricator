...
class Contact(private val utility: UtilityService, private val alpha: Alphanumeric, private val random: Random) {

  def fullName(setPrefix: Boolean, setSuffix: Boolean): String = {
    (setPrefix, setSuffix) match {
      case (`setPrefix`, `setSuffix`) if setPrefix && !setSuffix =>
        String.format("%s %s %s", prefix, firstName, lastName)
      case (`setPrefix`, `setSuffix`) if setPrefix && setSuffix =>
        String.format("%s %s %s %s", prefix, firstName, lastName, suffix)
      case (`setPrefix`, `setSuffix`) if setPrefix && setSuffix =>
        String.format("%s %s %s", firstName, lastName, suffix)
      case _ => firstName + " " + lastName
    }
  }

  def birthday(age: Int): String =
    DateTimeFormat.forPattern(DateFormat.dd_MM_yyyy.getFormat).print(DateTime.now() - age.years)

  def birthday(age: Int, format: DateFormat): String =
    DateTimeFormat.forPattern(format.getFormat).print(DateTime.now() - age.years)

  def eMail: String = {
    (firstName.toLowerCase + "_" + lastName.toLowerCase + alpha.numerify("###")).replaceAll(" ", "") + "@" + utility
  ...
  def height(cm: Boolean): String = {
    if (cm) alpha.randomDouble(1.50, 2.20).toString.substring(0, 4) + " cm"
    else alpha.randomInt(150, 220).toString + " m"
  }

  def weight(metric: Boolean): String =
    if (metric) alpha.randomInt(50, 110).toString + " kg" else alpha.randomInt(30, 90).toString + " lbs"
